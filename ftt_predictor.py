...
def extract_features(tap_intervals, tap_positions=None):
    ...
    if tap_positions:
        ...
        if tap_positions:
            # Debug information
            print("DEBUG x_coords:", x_coords[:5], "length:", len(x_coords))
            print("DEBUG y_coords:", y_coords[:5], "length:", len(y_coords))
            print("DEBUG unique x values:", len(set(x_coords)))
            print("DEBUG unique y values:", len(set(y_coords)))
            
            # Check for invalid values
            x_has_nan = any(np.isnan(x) for x in x_coords)
            y_has_nan = any(np.isnan(y) for y in y_coords)
            print("DEBUG x has NaN:", x_has_nan)
            print("DEBUG y has NaN:", y_has_nan)
            
            # Compute variance
            x_var = np.var(x_coords)
            y_var = np.var(y_coords)
            print("DEBUG x variance:", x_var)
            print("DEBUG y variance:", y_var)
            ...
            try:
                if x_var > 0 and y_var > 0:
                    ...
                    print("DEBUG correlation value:", corr_value)
                    features["corXY"] = 0.0 if np.isnan(corr_value) else corr_value
                else:
                    print("DEBUG zero variance detected, setting corXY to 0")
                    features["corXY"] = 0.0
            except Exception as e:
                print("DEBUG correlation calculation error:", str(e))
                features["corXY"] = 0.0
